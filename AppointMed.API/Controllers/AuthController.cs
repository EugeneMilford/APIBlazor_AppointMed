    ...
    public class AuthController : ControllerBase
    {
        ...
        public async Task<IActionResult> Register(UserDto userDto)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(userDto.Role) || !allowedRoles.Contains(userDto.Role))
                {
                    ModelState.AddModelError("Role", "Invalid role. Please select either 'User' or 'Administrator'.");
                    return BadRequest(ModelState);
                }

                var user = mapper.Map<ApiUser>(userDto);
                user.UserName = userDto.Email;
                user.Role = userDto.Role; // Sets the Role property
                ...
                // Assigns the selected role to Identity's role system
                await userManager.AddToRoleAsync(user, userDto.Role);

                logger.LogInformation($"User {userDto.Email} registered successfully with role {userDto.Role}");
